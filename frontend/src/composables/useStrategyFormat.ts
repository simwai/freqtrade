/**
 * Shared formatting utilities for strategy display logic.
...
function basisLabel(record: Record<string, unknown>): string {
  if (record.basis === 'registry') return 'no runs yet'
  return record.basis === 'benchmark' ? 'benchmark (fallback)' : 'last backtest'
}

function basisTooltip(record: Record<string, unknown>): string {
...
function propPassCount(record: Record<string, unknown>): number {
  const pf = record.prop_firms as Record<string, unknown> | undefined
  if (!pf) return -1
...
function propText(record: Record<string, unknown>): string {
  const pf = record.prop_firms as Record<string, unknown> | undefined
  if (!pf) return '—'
...
function propTitle(record: Record<string, unknown>, propSpec: Record<string, unknown>): string {
  const pf = record.prop_firms as Record<string, unknown> | undefined
  if (!pf) return ''
  const keys = Object.keys(pf)
...
function propClass(record: Record<string, unknown>): string {
  const pf = record.prop_firms as Record<string, unknown> | undefined
