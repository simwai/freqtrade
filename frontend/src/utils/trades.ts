/**
 * Compact trade representation with single-letter property names for compact serialization.
 * See the trade detail API for the full property mapping.
 */
...
export function tpPriceForTrade(t: CompactTrade, roi: Record<string, unknown> | null): number | null {
...
export function tlExitColor(t: CompactTrade, style?: string, mc?: Record<string, string>): string {
  ...
  return (EXIT_PAL as Record<string, string>)[key] || EXIT_PAL.unknown
}
...
export function markerItem(_params: unknown, api: { coord: (v: number[]) => number[]; value: (i: number) => number }): { type: string; shape: object; style: object; textContent?: object; textConfig?: object } {
  const p = api.coord([api.value(0), api.value(1)])
  const kind = api.value(2), color = String(api.value(3)), txt = api.value(4)
  const s = 7
...
export function tlTfMs(tf: string): number | null {
  ...
  if (!m) return null
  const multipliers: Record<string, number> = { m: 60e3, h: 3600e3, d: 86400e3 }
  return Number(m[1]) * multipliers[m[2].toLowerCase()]
}
