...
async def main():
    async with async_playwright() as p:
        ...
        # Check the global _tmTab reference
        check = await page.evaluate("""() => {
            return {
                global_tmTab: typeof window._tmTab,
                global_tmTab_value: window._tmTab,
                renderTrades_uses: renderTrades.toString().includes('_tmTab'),
            };
        }""")
        print(f"Check: {json.dumps(check, indent=2)}")
        
        # Create trade map instance and assign to global _tmTab
        await page.evaluate("""() => {
            window._tmTab = mountTradeMap('tm-tab');
            console.log('Created _tmTab, global:', window._tmTab === window._tmTab);
        }""")
        
        # Now fetch data and call renderTrades
        await page.evaluate("""() => {
            ...
            return fetch(`trades/${encodeURIComponent(key)}.json?v=${bust}`)
                ...
                .then(d => {
                    console.log('Data fetched:', d.strategy, d.trades?.length);
                    window.tradeCache = window.tradeCache || {};
                    window.tradeCache[key] = d;
                    // Call renderTrades - it should use the global _tmTab
                    renderTrades(d);
        ...
        print(f"\nTrade map state: {json.dumps(state, indent=2)}")
        ...
        await page.screenshot(path="dashboard_rendered2.png", full_page=True)
