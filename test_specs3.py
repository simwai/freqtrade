...
for name, fn in vars(mod).items():
    ...
    for p in sig.parameters.values():
        ...
        if p.default is p.empty:
            ...
            params.append({'name': p.name, 'default': default_val, 'type': ann_str})
    if ok and inputs:
        specs[name] = {'inputs': inputs, 'params': params}

for name in ['wavetrend', 'vqzl_zscore', 'bbwp', 'cdl_engulfing', 'hurst_exponent']:
    spec = specs.get(name, {})
    pcount = len(spec.get('params', []))
    print(f'{name}: inputs={spec.get("inputs")}, params_count={pcount}')
